...
def main():
    ...
    print(f"Scale: {total_params['s']:.4f}")
    
    # Verify Forward (Source -> Target) using params
    # manually apply params to source_points
    manual_aligned = apply_transform(source_points, total_params['s'], total_params['R'], total_params['t'])
    # Since Source is a subset of the Original mesh, it should match the subset of Target
    target_subset = target_points[indices]
    diff_fwd = manual_aligned - target_subset
    rmse_fwd = math.sqrt(np.mean(np.sum(diff_fwd**2, axis=1)))
    print(f"Forward RMSE (using params, subset check): {rmse_fwd:.8f}")
    
    # 4. Invert Params
    s_inv, R_inv, t_inv = ScaleAdaptiveICP.invert_transform(total_params['s'], total_params['R'], total_params['t'])
    
    print("\nInverted Transform (Target -> Source):")
    print(f"Scale: {s_inv:.6f} (Expected: {1/s_gt:.6f})")
    
    # Verify Inverse (Target -> Source) using params
    # Apply inverted params to target_points, should match the full centered_verts
    manual_inverse = apply_transform(target_points, s_inv, R_inv, t_inv)
    diff_inv = manual_inverse - centered_verts
    rmse_inv = math.sqrt(np.mean(np.sum(diff_inv**2, axis=1)))
